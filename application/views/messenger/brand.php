...
function processList(list_data){
	...
	if (list_data.list.length == 0){
		...
		for (var i in list_data.list){
			...
			var name 		= list_data.list[i].name;
			...
			if (from_email == list_data.me){
				html += '<div class="chat-conversation-user"><div style="height: 40px; width: 40px; float:left; margin-right: 10px">'+img+'</div>'+(event?event+' Organizer':to_name)+'</div>';
			} else {
				html += '<div class="chat-conversation-user"><div style="height: 40px; width: 40px; float: left; margin-right: 10px">'+img+'</div>'+(event?event+ ' Organizer':from_name)+'</div>';
			}
			html += '<div class="chat-conversation-excerpt"><span class="light-grey ml10 mt10 float-right">'+ago+'</span>'+message+'</div>';
			html += '</div>';
